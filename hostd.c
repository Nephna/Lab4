...
int executeQueue (struct resources* resources, struct queue* queue, struct queue* nextQueue) {
    ...
    while (queue->head != NULL) {
        ...
        if (pid == 0) {
            ...
            printf("PID: %d Arrival Time: %d Priority: %d Runtime: %d Address: %d Memory: %d\n",
                    process->pid, process->arrivalTime, process->priority, process->runTime, process->address, process->memory);
            execl(PROCESS, 0);
            exit(0);
        }
        // parent process
